...
resource "azurerm_storage_account" "media_storage" {
  ...
  name                            = "gsb-prodstoremedia${var.random_string}"
...
resource "azurerm_user_assigned_identity" "vi" {
  resource_group_name = var.resource_group_name
  location            = var.location
  name                = "gsb-prod-ua-ident-${var.random_string}"
}
...
resource "azurerm_resource_group_template_deployment" "vi" {
  ...
  parameters_content = jsonencode({
    "name"                      = { value = "gsb-prod-avi-${var.random_string}" },
...
output "vi_name" {
  value = "gsb-prod-avi-${var.random_string}"
}

output "vi_id" {
  value = "/subscriptions/${var.subscription_id}/resourceGroups/${var.resource_group_name}/providers/Microsoft.VideoIndexer/accounts/gsb-prod-avi-${var.random_string}"
}